...
if __name__ == "__main__":
	...
	print("---------------------------------")
	print("Mapper Output")
        for row in closest.collect():print(str(row[0])+","+row[1][0].printPoint()+","+str(row[1][1]))
	print("---------------------------------")
        pointStats = closest.reduceByKey(add_tuples_values)
	print("---------------------------------")
	print("Reducer Output")
	for row in pointStats.collect():print(str(row[0])+","+row[1][0].printPoint()+","+str(row[1][1]))
	print("---------------------------------")
	print("---------------------------------")
	print("New Centroids")
	newPoint=pointStats.map(lambda p:(p[0],p[1][0].getAverage(p[1][1])))
        for row in newPoint.collect():print(str(row[0])+","+row[1].printPoint())
	print("--------------------------------")
	newCentroids=[]
	for row in newPoint.collect():newCentroids.append(row[1])
        #oldCentroids=newCentroids
